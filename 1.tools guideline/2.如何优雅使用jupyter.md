...
2. cd 到 R安装路径，例如 cd C:\Program Files\R\R-4.2.3\bin
...
5. 可能会报以下错误
   ...
   ```
   devtools::install_github('IRkernel/IRkernel') Error in loadNamespace(i, c(lib.loc, .libPaths()), versionCheck = vI[[i]]) : there is no package called 'stringi'
   ```
...
3. ModuleNotFoundError: No module named 'notebook.notebookapp'
...
4. TypeError: warn() missing 1 required keyword-only argument: 'stacklevel'
...
6. [在Pycharm中使用notebook](https://blog.csdn.net/xiemanR/article/details/71837385)
