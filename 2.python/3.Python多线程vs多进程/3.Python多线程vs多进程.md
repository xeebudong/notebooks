...
> 数据科学，一般用concurrent.futures来实现多进程

1. [python多线程多进程concurrent.futures详细总结](https://blog.csdn.net/qq_42680088/article/details/99776825)
...
2. 示例代码
   ...
   > 通过以上方式就可以多个进程同时运行f函数, future.result() 是函数返回的i, max_worker 是进程/线程数, 默认为 CPU 核心数, f 为要执行的函数, i 为传给 f 的参数, 其中 ProcessPoolExecutor 多进程可以替换为 ThreadPoolExecutor 多线程
   > ————————————————
   > 版权声明：本文为CSDN博主「命数如织-当如磐石」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
   > 原文链接：https://blog.csdn.net/qq_42680088/article/details/99776825
